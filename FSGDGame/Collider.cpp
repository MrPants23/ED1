...
namespace Behaviors
{
	...
	{
		pair<bool, Sphere>* spherePair = (pair<bool, Sphere>*)traversalStruct.m_pData;
		Sphere& sphere = spherePair->second;
	...
	{
		pair<bool, Segment>* segPair = (pair<bool, Segment>*)traversalStruct.m_pData;
	...
	BVHTemplate* BVHTemplate::Load(const char* pGDMeshFile, const char* indentifier)
	{
		...
		resultPtr->m_pAabbBvh = new AabbBvh;
		resultPtr->m_PointsHandle = std::move(pointsHandle);
		resultPtr->m_TriNormalsHandle = std::move(triNormalHandle);
		resultPtr->m_IndicesHandle = std::move(indicesHandle);
		...
		for(unsigned int i = 0; i < bvhNodesHandle.GetContent()->size(); ++i)
		{
			...
			else
			{
				...
				AabbBvhLeafNode* newLeafPtr = newLeafDataPtr->CreateLeafNode(resultPtr->m_pAabbBvh);
				pNodeArr[i] = newLeafPtr;
			}
		}

		resultPtr->m_pAabbBvh->SetRootNode( (EDTreeInternalNode*)pNodeArr[bvhNodesHandle.GetContent()->size()-1] );

		std::vector< EDTreeLeafNode* >& leafList = resultPtr->m_pAabbBvh->GetLeaves();
	...
	{
		Traverse( m_pAabbBvh->GetRoot(), traversalStruct );
	}
	...
	void BVHTemplate::Traverse( const EDTreeNode* node, BVHTraversalStruct& traversalStruct )
	{
		if( node->IsLeaf() )
		{
			...
			if( traversalStruct.m_PerNodeFunc( *leafPtr->GetBounds(), traversalStruct ) )
			{
				...
				for(unsigned int i = 0; i < triCount; ++i)
				{
					...
					traversalStruct.m_PerTriangleFunc( m_PointsHandle.GetContent()->operator[](m_IndicesHandle.GetContent()->operator[]( triIndex ).index0),
													   m_PointsHandle.GetContent()->operator[](m_IndicesHandle.GetContent()->operator[]( triIndex ).index1),
													   m_PointsHandle.GetContent()->operator[](m_IndicesHandle.GetContent()->operator[]( triIndex ).index2),
													   m_TriNormalsHandle.GetContent()->operator[]( triIndex ),
													   traversalStruct );
				}
			}
		}
		else
		{
			...
			if( traversalStruct.m_PerNodeFunc( *pInternal->GetInternalNodeData(), traversalStruct ) )
			{
	...
	void Collider::PopulateMessageMap()
	{
		...
		AddMessageHandler( MSG_GET_COLLIDER, OnGetCollider );
		...
		AddMessageHandler( MSG_INTERSECT_SEGMENT, OnIntersectSegment );
	}
	...
	void Collider::OnAttributeChanged( IBehavior* pInvokingBehavior, IMessage* msg )
	{
		...
		if( message->value == COLLIDER_ATTRIB )
		{
			pThis->colliderObject = ((Attribute<ColliderDescriptor>*)pThis->GetAttribute(COLLIDER_ATTRIB))->value.object;
	...
	void Collider::OnIntersectSegment( IBehavior* pInvokingBehavior, IMessage* msg )
	{
		...
		if( pThis->GetColliderType() == MESH )
		{
			...
			traversalStruct.m_PerTriangleFunc = SegmentToTriangle;
			traversalStruct.m_PerNodeFunc = SegmentToNodeAabb;
			traversalStruct.m_pData = &localSegPair;
			...
			if( localSegPair.first == true )
			{
				...
				TranslateVector( message->value.second.end, localSegPair.second.end, pThis->gameObject->GetWorldTransform() );
			}
		}
	}
	...
	void Collider::OnCollide( IBehavior* invokingBehavior, IMessage* message )
	{
		...
		MessageT<Collider*> getColliderMsg(MSG_GET_COLLIDER, 0);
	...
	bool Collider::ColliderSphereToMesh(Collider* objA, Collider* objB, CollisionContact& collisionContact)
	{
		...
		Sphere worldSphere = pSphereObj->GetLocalSphere();
		worldSphere.Transform( objA->gameObject->GetWorldTransform() );

		Sphere objSpaceSphere = worldSphere;
		objSpaceSphere.InverseTransform( objB->gameObject->GetWorldTransform() );

		BVHTraversalStruct traversalStruct;
		pair<bool, Sphere> spherePair( false, objSpaceSphere );
		traversalStruct.m_PerNodeFunc = SphereToNodeAabb;
		traversalStruct.m_PerTriangleFunc = SphereToTriangle;
		traversalStruct.m_pData = &spherePair;
		...
		float3 tempV;
		RotateVector(tempV, collisionContact.m_ContactNormal, objB->gameObject->GetWorldTransform() );
