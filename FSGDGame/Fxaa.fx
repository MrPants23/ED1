...
float4 FxaaFS(TwoDVertexOut inVert) : COLOR0
{
	...
	float3 colorCenter = tex2D(sourceSampler, texCoord).rgb;
	float3 colorN = tex2DOffset(sourceSampler, texCoord, float2(0, -1)).rgb;
	float3 colorS = tex2DOffset(sourceSampler, texCoord, float2(0, 1)).rgb;
	float3 colorE = tex2DOffset(sourceSampler, texCoord, float2(1, 0)).rgb;
	float3 colorW = tex2DOffset(sourceSampler, texCoord, float2(-1, 0)).rgb;
	...
	float3 colorNW = tex2DOffset(sourceSampler, texCoord, float2(-1, -1)).rgb;
	float3 colorNE = tex2DOffset(sourceSampler, texCoord, float2(1, -1)).rgb;
	float3 colorSW = tex2DOffset(sourceSampler, texCoord, float2(-1, 1)).rgb;
	float3 colorSE = tex2DOffset(sourceSampler, texCoord, float2(1, 1)).rgb;
	...
	for(int i = 0; i < FXAA_SEARCH_STEPS; ++i)
	{
		...
		if(!doneN) 
			lumaEndN = calcLuminance(tex2D(sourceSampler, posN.xy).xyz);
		if(!doneP) 
			lumaEndP = calcLuminance(tex2D(sourceSampler, posP.xy).xyz);
    ...
    float3 rgbF = tex2D(sourceSampler, float2(
...
technique PostProcess
{
    pass P0
    {
        ...
        pixelShader  = compile ps_3_0 FxaaFS();
        ZEnable = false;
    }
}
