...
namespace EDUtilities
{
    ...
    GDTextureObject::GDTextureObject(std::ifstream &fin)
    {
        Read(fin);
    }
    ...
    const std::vector< std::string >& GDTextureObject::GetTextureFileNames(void) const
    {
        return textureFileNames;
    }

    void GDTextureObject::SetTextureFileNames( const std::vector< std::string >& names )
    {
        textureFileNames = names;
    }

    void GDTextureObject::Read(std::ifstream &fin)
    {
        int count;
    ...
    void GDTextureObject::Write(std::ofstream &fout) const
    {
        ...
        {
            namelen = textureFileNames[i].length();
            fout.write( (char*)&namelen, sizeof(namelen) );
            fout.write( textureFileNames[i].c_str(), namelen );
        }
    }

    void GDTextureObject::WriteXML(std::ofstream &fout) const
    {
	...
	{
		std::string fileName = ContentManager::theContentPath + "/gdt_";
		fileName += name;
		fileName += ".xml";
	...
	void GDTextureObject::OpenXML(const char* fileName)
	{
		...
		while( elementPtr != 0 )
		{
			std::string textureFileName = elementPtr->GetText();
