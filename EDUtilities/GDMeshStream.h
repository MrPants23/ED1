...
{
	ContentHandle<GDAttribute<TriVertIndices>> triVertIndicesHandle;
	std::string name;
	std::string fileName;
...
public:
	...
	void OpenFile(void)
	{
		...
		fileStream.seekg( 4*3*size, ios_base::cur );
		fileStream.read( (char*)&numAttributes, sizeof(numAttributes) );
		attributeOffset = (unsigned int)fileStream.tellg();
		
		triVertIndicesHandle = ContentManager::Load<GDAttribute<TriVertIndices>>( fileName.c_str() );
	}
	...
	ContentHandle<GDAttribute<TriVertIndices>> GetIndicesHandle(void)
	{	
		OpenFile();
	...
	std::ifstream* GetAttributeTypeListPtr(const char* attributeTypeName, const char* indentifier = 0)
	{
		OpenFile();
		...
		while( !fileStream.eof() )
		{
			...
			fileStream.seekg( byteCount*itemCount, ios_base::cur );
		}
	...
	ContentHandle<GDAttribute<AttributeType>> ReadAttribute(const char* indentifier = 0)
	{
		ContentHandle<GDAttribute<AttributeType>> handle;

		if( fileName.length() != 0 )
			handle = ContentManager::Load<GDAttribute<AttributeType>>( fileName.c_str(), indentifier );
...
public:
	...
	void LoadDocument(void)
	{
		...
		docPtr = new (TiXmlDocument)((ContentManager::theContentPath + fileName).c_str());
		if( !docPtr->LoadFile() )
		{
			ReleaseSource();
			return;
		...
		if(pTriListElement != 0)
			triVertIndicesHandle = ContentManager::LoadXML<GDAttribute<TriVertIndices>>( pTriListElement );
	...
	ContentHandle<GDAttribute<AttributeType>> ReadAttribute(const char* indentifier = 0)
	{
		...
		if( fileName.length() != 0 )
			handle = ContentManager::LoadXML<GDAttribute<AttributeType>>( fileName.c_str(), indentifier );
	...
	{	
		LoadDocument();
		if( triVertIndicesHandle.GetContent() != 0 )
			return (triVertIndicesHandle.GetContent()->size() == 0 ? 0 : (*triVertIndicesHandle.GetContent())[0].indices);
