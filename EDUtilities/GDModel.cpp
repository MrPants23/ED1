...
namespace EDUtilities
{
    ...
    GDModelPart::GDModelPart(std::ifstream &fin)
    {
        parentIndex = -1;
        localTransform.makeIdentity();
        Read( fin );
    ...
    void GDModelPart::Read(std::ifstream &fin)
    {
        int count = 0;
    ...
    void GDModelPart::Write(std::ofstream &fout) const
    {
...
//        fout.write( (char*)localTransform.ma, sizeof(localTransform) );
    ...
    GDModel::GDModel( std::ifstream &fin )
    {
        Read( fin );
    }
    ...
    void GDModel::SetModelParts(const std::vector<GDModelPart> &_modelParts)
    {
    ...
    void GDModel::Read(std::ifstream &fin)
    {
        int count = 0;
        fin.read( (char*)&count, sizeof(count) );
		char* tempName = new char[count+1];
    ...
    void GDModel::Write(std::ofstream &fout) const
    {
        size_t count = name.length();
        fout.write( (char*)&count, sizeof(count) );
        fout.write( name.c_str(), count );
	...
	void GDModel::WriteXML(std::ofstream &fout) const
	{
		char buffer[128];
	...
	{
		std::string fileName = ContentManager::theContentPath + "/gdmdl_";
		fileName += name;
		fileName += ".xml";
