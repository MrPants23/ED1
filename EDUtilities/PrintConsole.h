...
namespace EDUtilities
{
	...
	public:
		...
		enum PrintPageNames{PPN_Render = 0, PPN_Input, PPN_ED1Solutions, PPN_Memory, PPN_Total};
	private:
	...
	public:
		...
		void AddString(PrintPageNames page, float3color&& color, const char *sz, ...)
		{
			...
			char buffer[PrintPage::MaxStringLength];
			vsnprintf_s(buffer, PrintPage::MaxStringLength, sz, ap);
		...
		template <typename float3color>
		void AddSolution(const char *szClass, unsigned int const labIndex, float3color&& color, 
			const char *sz, ...)
		{
			va_list ap;
			va_start(ap, sz);
			...
			// pass that string off to where it should be stored
			pages[PPN_ED1Solutions].AddSolution(szClass, labIndex - 1, 
				std::forward<float3>(color), buffer);
			va_end(ap);
		}
		/// This method will create the D3D font to be used for rendering.
        /// \param pDevice - The D3D device to be used to create this font.
		void CreateRenderFont(IDirect3DDevice9 *pDevice);
