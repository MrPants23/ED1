...
namespace EDGOCA
{
	...
	public:
		...
		inline const float4x4& GetLocalTransform(void) const { return localTransform; }
		...
		virtual void TranslateLocal(const float3& delta);
		virtual void TranslateGlobal(const float3& delta);
		...
		virtual void SetLocalTransform(const float4x4& transform);
		virtual void SetLocalPosition(const float3& position);
		...
		inline const set<TransformNode*>& GetChildren(void) const { return children; }
	...
	private:
		...
		set<TransformNode*> children;

		const float4x4& GetWorldTransformSolution(void);
