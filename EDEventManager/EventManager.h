...
#ifdef _DEBUG
#define EM_DEBUG_HASHING 1
#else
#define EM_DEBUG_HASHING 0
#endif
...
namespace EDEventManager
{
	...
	typedef void (*EM_CALLBACK)(unsigned int eventId, void *eventData, void *listener);
	...
	struct PriorityEvent
	{
		...
		PriorityEvent(unsigned int eventId, void *eventData = nullptr, 
	...
	public:
		...
		void PostEvent(const std::string &eventName, void *data = nullptr, 
			EventPriority priority = EM_PRIORITY_NORMAL, EventFlag flags = EM_FLAG_NONE);
		...
		/// \param priority - Priority of this event. See EM_PRIORIRTY_* enums
		/// \param flags - Flags for processing this event. See EM_FLAG_* enums.
		void PostEvent(unsigned int eventId, void *data = nullptr, 
			EventPriority priority = EM_PRIORITY_NORMAL, EventFlag flags = EM_FLAG_NONE);
		...
		/// Performs various checks on the generation of hashes
		/// \param eventName - Name of the event
		/// \param hash - Hash of eventName
		void DebugCheckHash(const std::string &eventName, unsigned int hash);
